...
# gif of log(nuclear power ouput) vs log(country population)
#
#
...
for n in range(49):
    ...
    output = [np.log((x + 1)) for x in output_by_country]
    df['output'] = output
    pop = list(df_pop[str(year)])
    pop = [np.log(x) for x in pop]
    df['population'] = pop
